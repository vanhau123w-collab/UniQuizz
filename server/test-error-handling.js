// server/test-error-handling.js - Test Error Handling Implementation
...
// Test configuration
const TEST_CONFIG = {
  mongoUri: process.env.MONGODB_URI || 'mongodb://localhost:27017/test_rag_search',
...
class ErrorHandlingTester {
  ...
  async testPerformanceMonitoring() {
    ...
    await this.runTest('Performance Monitoring - Operation Tracking', async () => {
      ...
      if (!operation || operation.duration < 100) {
  ...
  async testLogging() {
    ...
    await this.runTest('Search Logger - Search Operation Logging', async () => {
      const logger = searchLogger.createSearchLogger('test-search-1', 'test query', { userId: TEST_CONFIG.testUserId });
    ...
    await this.runTest('Search Logger - Error Logging', async () => {
      const logger = searchLogger.createSearchLogger('test-search-2', 'test query', { userId: TEST_CONFIG.testUserId });
  ...
  async testSystemHealth() {
    ...
    await this.runTest('Error Handler Wrapper', async () => {
      const mockReq = { userId: TEST_CONFIG.testUserId };
      const mockRes = {
      ...
      try {
        ...
        // The wrapper should handle the error, not throw it
      } catch (error) {
        throw new Error('Error handler wrapper should catch and handle errors');
      }
    });
  }
  ...
  async runAllTests() {
    ...
    console.log(`${colors.cyan}🧪 RAG Search Error Handling Test Suite${colors.reset}`);
    console.log(`${colors.cyan}${'='.repeat(60)}${colors.reset}\n`);

    try {
      // Connect to test database
      await mongoose.connect(TEST_CONFIG.mongoUri);
      console.log(`${colors.green}✅ Connected to test database${colors.reset}\n`);
